...
namespace HyForce.Tabs
{
    public class ProtocolLabTab : ITab
    {
        ...
        public void Render()
        {
            ...
            ImGui.TextColored(
                live ? new Vector4(0.1f, 1f, 0.4f, 1f) : new Vector4(0.7f, 0.3f, 0.3f, 1f),
        ...
        private void RenderFuzzer()
        {
            ...
            ImGui.TextWrapped("Flips random bits in the NEXT outgoing C->S packet, then sends it. Observe whether the server disconnects, sends an error opcode, or accepts it silently (silent acceptance = missing validation).");
            ImGui.Spacing();
            ...
            ImGui.Spacing();
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "What to look for:");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Kicked / disconnected  ->  server validates packet integrity");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Graceful error reply   ->  server has error handling");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Nothing happens        ->  server silently drops bad packets (good)");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Server crashes / hangs ->  potential DoS vulnerability (report this!)");
        }

        private void RenderReplay()
        {
            ImGui.TextColored(new Vector4(0.9f, 0.7f, 0.2f, 1f), "Packet Replay");
            ImGui.TextWrapped("Resend a previously captured packet verbatim, or inject custom hex bytes. Tests replay-attack resistance and idempotency.");
            ImGui.Spacing();
            ...
            ImGui.SameLine();
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "(replays whatever the last client->server packet was)");
            ...
            {
                string clean = _replayHex.Replace(" ", "").Trim();
                if (clean.Length > 0 && clean.Length % 2 == 0)
                {
                    _pipe.PktForge(clean);
            ...
            ImGui.Spacing();
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "What to look for:");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Replay accepted = no replay protection (e.g. duplicate item use, duplicate purchase)");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Nonce / sequence rejection = server has replay protection");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  - Crash on custom hex = unsafe deserialization / missing length check");
        }

        private void RenderRateLimit()
        {
            ImGui.TextColored(new Vector4(0.9f, 0.7f, 0.2f, 1f), "Rate-Limit Prober");
            ImGui.TextWrapped("Floods the last captured C->S packet N times over T milliseconds to find where the server starts dropping, throttling, or kicking.");
            ImGui.Spacing();
            ...
            ImGui.Spacing();
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "Recommended test sequence:");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  10/1000ms -> 50/1000ms -> 100/1000ms -> 500/1000ms");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  Watch the log for disconnects or error packets coming back.");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "  Also watch server CPU via task manager if you have server access.");
        }

        private void RenderPcap()
        {
            ImGui.TextColored(new Vector4(0.9f, 0.7f, 0.2f, 1f), "PCAP Export");
            ImGui.TextWrapped("Write all captured UDP traffic to a .pcap file. Open in Wireshark for deep protocol analysis with its QUIC dissector.");
            ImGui.Spacing();
            ...
            ImGui.Spacing();
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "In Wireshark: Edit -> Preferences -> Protocols -> QUIC");
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "Add your SSLKEYLOGFILE path under 'TLS key log file' to decrypt inline.");
        }

        private void RenderSeq()
        {
            ImGui.TextColored(new Vector4(0.9f, 0.7f, 0.2f, 1f), "QUIC Sequence Number Tracker");
            ImGui.TextWrapped("QUIC packet numbers must be strictly monotonically increasing. Non-monotonic or duplicate packet numbers indicate a client-side bug or a server that does not properly reject replayed/reordered packets.");
            ImGui.Spacing();
        ...
        private void RenderOpcodeMap()
        {
            ...
            ImGui.TextWrapped("Catalogues unique first-byte values seen in captured packets. After decryption works, this maps the server's message type space.");
            ImGui.Spacing();
            ...
            if (ImGui.BeginTable("##opmap", 3, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg))
            {
                ...
                {
                    ImGui.TableNextRow();
                    ImGui.TableSetColumnIndex(0); ImGui.Text($"0x{op:X2}  ({op})");
                    ImGui.TableSetColumnIndex(1); ImGui.Text($"{cnt}");
        ...
        private void RenderTiming()
        {
            ...
            ImGui.TextWrapped("Approximate round-trip times from C->S send to next S->C receive. Useful as baseline - large spikes during rate-limit tests indicate server load.");
            ImGui.Spacing();
